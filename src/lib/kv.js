[04/07/1405 08:30 ب.ظ] Mmd: // All persistent state lives in Cloudflare KV (BOT_KV binding).
...
export async function getOrCreateUser(kv, from, referredBy) {
  ...
  if (user.username) {
    await kv.put(
      index:username:${user.username.toLowerCase()},
      String(user.id)
    );
  }
...
export async function removeFromIndex(kv, key, id) {
  ...
  const next = idx.filter((x) => Number(x) !== normalizedId);
...
[04/07/1405 08:30 ب.ظ] Mmd: export async function getTexts(kv) {
...
export async function saveProduct(kv, product) {
  if (!product.id) {
    product.id = await nextId(kv, "seq:products");
  }
...
export async function listProductsByCategory(kv, categoryId) {
  ...
  const products = await Promise.all(
    ids.map((id) => getProduct(kv, id))
  );

  return products.filter(
    (p) => p && p.category_id === categoryId
  );
}
...
export async function savePanel(kv, panel) {
  if (!panel.id) {
    panel.id = await nextId(kv, "seq:panels");
  }
...
export async function createOrder(kv, order) {
  ...
  await addToIndex(
    kv,
    index:orders:user:${order.user_id},
    order.id
  );
...
export async function getUserOrders(kv, userId) {
  const ids = await getIndex(
    kv,
    index:orders:user:${userId}
  );

  return Promise.all(
    ids.map((id) => getOrder(kv, id))
  );
}
...
[04/07/1405 08:30 ب.ظ] Mmd: await addToIndex(
    kv,
    index:services:user:${service.user_id},
    service.id
  );
...
export async function getUserServices(kv, userId) {
  const ids = await getIndex(
    kv,
    index:services:user:${userId}
  );

  return Promise.all(
    ids.map((id) => getService(kv, id))
  );
}
...
export async function createTicket(kv, ticket) {
  ...
  ticket.messages = [
    {
      from: "user",
      text: ticket.first_message,
      at: Date.now(),
    },
  ];

  delete ticket.first_message;
  ...
  await addToIndex(
    kv,
    index:tickets:user:${ticket.user_id},
    ticket.id
  );
...
export async function saveTicket(kv, ticket) {
  ...
  // هر تیکتی که بسته نیست باید در لیست تیکت‌های باز باشد.
...
[04/07/1405 08:30 ب.ظ] Mmd: export async function getStats(kv) {
  return getJSON(
    kv,
    "stats:global",
    {
